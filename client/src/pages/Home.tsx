    ...
    <div>
      ...
      <section style={{ marginTop: "1.5rem" }}>
        ...
        <p>
          ...
          {health.status === "ok" && (
            <span style={{ color: "#5dd58c" }}>✅ {health.message}</span>
          )}
          {health.status === "error" && (
            <span style={{ color: "#ff6b6b" }}>❌ {health.message}</span>
          )}
        </p>
      </section>
      ...
      <section style={{ marginTop: "1.5rem" }}>
        <h2>確認されたコレクション</h2>
        {health.status !== "ok" ? (
          <p style={{ color: "#888" }}>PocketBase に接続できたら表示されます</p>
